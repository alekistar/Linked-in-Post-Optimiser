          ...
          <div className="flex flex-col items-center justify-center min-h-[60vh] animate-in fade-in duration-500">
              <Card glow="purple" className="p-8 max-w-md w-full space-y-6">
                  <div className="flex flex-col items-center text-center">
                    ...
                    <h2 className="text-2xl font-bold text-white">Connect LinkedIn</h2>
                  ...
                  <div className="space-y-4">
                    <div>
                        <label className="text-xs font-bold text-slate-500 uppercase mb-1 block">LinkedIn Profile URL</label>
                        <div className="relative">
                            ...
                            <input 
                                ...
                                className="w-full bg-black/50 border border-white/10 rounded-lg py-2.5 pl-10 pr-4 text-sm text-white focus:border-neon-blue focus:ring-1 focus:ring-neon-blue outline-none transition-all"
                            />
                        </div>
                        {error && <p className="text-red-400 text-xs mt-1 flex items-center gap-1"><AlertTriangle className="w-3 h-3"/> {error}</p>}
                    </div>

                    <div className="bg-yellow-500/10 border border-yellow-500/20 rounded p-3 text-xs text-yellow-200 text-left flex gap-2">
    ...
    <div className="space-y-6 animate-in slide-in-from-bottom-5 duration-500">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
                ...
                <div className="flex items-center gap-2">
                    <p className="text-xs text-slate-500 font-mono">Connected: {profileUrl.replace('https://linkedin.com/in/', '@')}</p>
                    <CheckCircle2 className="w-3 h-3 text-green-500" />
                </div>
            </div>
          </div>
          <div className="flex gap-2">
            <Button variant="ghost" onClick={handleDisconnect} className="text-xs h-8 px-3 text-red-400 hover:text-red-300 hover:bg-red-500/10">
                Disconnect
            </Button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex bg-white/5 rounded p-1">
                    <button 
                        ...
                        className={`text-xs px-3 py-1 rounded transition-all ${comparisonMode === 'engagement' ? 'bg-neon-purple text-white' : 'text-slate-400 hover:text-white'}`}
                    >
                        Engagement
                    </button>
                    <button 
                        onClick={() => setComparisonMode('reach')}
                        className={`text-xs px-3 py-1 rounded transition-all ${comparisonMode === 'reach' ? 'bg-neon-cyan text-black font-bold' : 'text-slate-400 hover:text-white'}`}
                    >
                        Reach
                    </button>
                </div>
                    ...
                    return (
                        <div key={item.tone} className="relative group">
                            <div className="flex justify-between text-sm mb-2">
                                <span className="font-bold text-slate-200">{item.tone} Tone</span>
        ...
        <Card className="p-6 flex flex-col">
          <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-white">Daily Growth</h3>
              <select className="bg-black/40 border border-white/10 rounded text-xs text-slate-400 px-2 py-1 outline-none focus:border-neon-cyan">
          ...
          <div className="h-48 flex items-end justify-between gap-2 mt-auto">
            {growthData.map((h, i) => (
              <div key={i} className="w-full bg-white/5 hover:bg-neon-cyan/20 rounded-t-lg transition-all relative group cursor-pointer">
                <div 
                ...
                />
                <div className="opacity-0 group-hover:opacity-100 absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 text-xs text-white px-2 py-1 rounded pointer-events-none transition-opacity">
                    {Math.floor(h * 10)} views
                </div>
              </div>
            ))}
          </div>
          <div className="flex justify-between mt-4 text-xs text-slate-500 font-mono border-t border-white/5 pt-2">
